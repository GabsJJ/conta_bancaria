...
public class ContaController implements ContaRepository {
	...
	public void sacar(int numero, float valor) {
		...
		if (conta.isPresent()) {
			if(conta.get().sacar(valor) == true)
				System.out.printf("\nO Saque no valor de %s, foi efetuado com sucesso", formatarMoeda.format(valor));
		} else
			System.out.printf("\nA Conta número %d não foi encontrada", numero);
	...
	public void depositar(int numero, float valor) {
		...
		if (conta.isPresent()) {
			...
			System.out.printf("\nO Depósito no valor de %s, foi efetuado com sucesso", formatarMoeda.format(valor));
		} else
			System.out.printf("\nA Conta número %d não foi encontrada", numero);
	...
	public void transferir(int numeroOrigem, int numeroDestino, float valor) {
		NumberFormat formatarMoeda = NumberFormat.getCurrencyInstance();
		Optional<Conta> cOrigem = buscarNaCollection(numeroOrigem);
		Optional<Conta> cDestino = buscarNaCollection(numeroDestino);

		if (cOrigem.isPresent() && cDestino.isPresent()) {
			if(cOrigem.get().sacar(valor) == true) {
				cDestino.get().depositar(valor);
				System.out.printf("\nA tranferência no valor de %s, da Conta número %d para a Conta número %d foi efetuado com sucesso", formatarMoeda.format(valor), numeroOrigem, numeroDestino);
			}
		} else
			System.out.printf("\nA Conta número %d não foi encontrada", numero);
